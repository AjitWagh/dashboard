...
class Data(db.Model):
  # Date = db.Column(db.Integer, primary_key = True)
   # name = db.Column(db.String(100))
    ...
    def __init__(self, Date, Month, JIRA_URL, Task, Deliverable, Dashboard_URL, BusinessValue, Stakeholders, Designation, id):
...
def Index():
    ...
    return render_template("tracker.html", tasks = all_data)
...
def insert():
    ...
    if request.method == 'POST':
        ...
        my_data = Data(Date, Month,JIRA_URL, Task, Deliverable, Dashboard_URL, BusinessValue, Stakeholders, Designation)
        db.session.add(my_data)
        db.session.commit()
